...
fun canBeMadeSafeWithDampener(levels: List<Int>): Boolean {
    // Try removing each level one by one and check if the remaining sequence is safe
...
fun countSafeReportsWithDampener(input: String): Int {
    ...
    return reports.count { isSafeReport(it) || canBeMadeSafeWithDampener(it) }
}
...
fun main() {
    ...
    val safeReports = countSafeReportsWithDampener(input)
    println("The number of safe reports with the Problem Dampener is: $safeReports")
}
